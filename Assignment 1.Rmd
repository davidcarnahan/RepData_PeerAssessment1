...
The easiest form of imputation would be to replace the NA with the mean of the interval from other days that do not have a missing value. I will round to whole integers for uniformity of the data.
...
activity3 <- select(activity2, date, interval, impSteps)
```

Here are the data with the imputation applied.
...
I need to prep the data for average steps by day with the new imputed dataset so I can then proceed to the graphing requirement.
...
Now I can rerun the previous code to generate a histogram of the new imputed dataset.
[Need to investigate why the number of steps per day seemt to be significantly more]
...
I need to explore the sum of the imputed values for the intervals that previously had missing values to get an idea of the magnitude of change I should expect.

```{r verify, echo=TRUE, results=TRUE}
actImpute <- activity2[ (is.na(activity2$steps)), ] ## subset the intervals with missing data
actImpute <- actImpute %>%
  group_by(interval) %>%
  summarise(sum=sum(impSteps))
head(actImpute, 500)
sum(activity2$impSteps)
sum(activity$steps, na.rm = TRUE)
str(dmelt)
str(dmelt2)  ## the problem appears to be with dmelt2 ... it looks like it doubled the count
```
